...
import { API, fetch, shorteneds } from "@/store";
...
async function addShortenedURL() {
  ...
  try {
    res = await API.post<void>(`/url/${newShort.value}`, {
      dest: newURL.value,
  ...
  if (res && res.status === 200) {
    ...
    alert(
      `Erreur lors de l'ajout : ${
        res ? ("error" in res ? res.error : res.status) : "Impossible d'effectuer la requête."
      }`,
    );
...
async function refreshSelected() {
  ...
  const stats = await API.get<Stat[]>(`/stats/${selectedShortenedURL.value.shortened}`, {
    params: {
      password: password.value.hashed,
    },
  });
